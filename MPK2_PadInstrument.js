...
PadInstrument.handleNavKeys = function(data1) {
  if (data1 == NAVKeys.LEFT) shifted ? cursorTrack.selectPrevious() : cursorDevice.selectPrevious();
  else if (data1 == NAVKeys.RIGHT) shifted ? cursorTrack.selectNext() : cursorDevice.selectNext();
  else if (data1 == NAVKeys.UP) shifted ? cursorDevice.switchToNextPreset() : cursorDevice.nextParameterPage();
  else if (data1 == NAVKeys.DOWN) shifted ? cursorDevice.switchToPreviousPreset() : cursorDevice.previousParameterPage();
}


PadInstrument.init = function() {
  if (displayHelpText) host.showPopupNotification("Pads: Instrument/Drum Rack Mode");
...
PadInstrument.cursorTrackColorObs = function(red, green, blue) {
  ...
  if (usingDrumMachine) PadInstrument.lightPadsForDrumRack(CursorTrackColor);
  else lightAllPads(CursorTrackColor, "Off");
}
...
PadInstrument.lightPadsForDrumRack = function(color) {
  ...
  for (var x = 36; x < 100; x++) {
    if (drumKeys[x] == false) MidiOut += " " + uint7ToHex(padColors['Off']);
    else MidiOut += " " + uint7ToHex(color);
  }
  MidiOut += " F7";
